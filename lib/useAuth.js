...
export function useAuth() {
  ...
  // ── SIGN-UP confirmation: 6-digit OTP (cross-device) ────────
  // Verify the code from the signup confirmation email (type "signup").
  ...
  // ── FORGOT PASSWORD: recovery via 6-digit OTP ───────────────
  // 1) email a recovery code, 2) verify it (type "recovery") → session,
  ...
  return { user, isAdmin, loading, signIn, signUp, verifyOtp, resendOtp, verifyTokenHash,
    sendRecovery, verifyRecoveryOtp, updatePassword, signOut, demo: !hasSupabase };
}
